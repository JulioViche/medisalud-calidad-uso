...
\scenario{4}{Usuario, tarea y contexto de uso · desarrollo aplicado}
...
Se eligieron los tres procesos solicitados por la guía: registro de una nota de evolución, agendamiento de una cita y facturación de una consulta con seguro. La selección es coherente con el diagnóstico inicial porque reúne continuidad clínica, acceso del paciente e impacto financiero.
...
\evidence{Las tareas se implementan como eventos tipados en \texttt{apps/analytics/simulation/generator.py}; sus campos normalizados se almacenan en \texttt{apps/data/simulated/events.csv}.}


\subsection{Fase 5: mapeo y priorización de características}

\scenario{5}{Matriz tarea--característica--prioridad · desarrollo aplicado}
...
\evidence{La matriz priorizada se vincula con los tipos de evento definidos en \texttt{apps/analytics/simulation/generator.py} y con el catálogo de medidas de \texttt{apps/analytics/metrics/calculator.py}.}


\subsection{Fase 6: diseño formal de métricas}

\scenario{6}{Fichas ISO/IEC 25022 y criterios de decisión · desarrollo aplicado}
...
Los umbrales son criterios académicos derivados de los requisitos del caso y deben aprobarse con responsables clínicos, financieros y legales antes de emplearse en producción. También es necesario mantenerlos en configuración versionada. Si quedan dispersos en código, un cambio no controlado alteraría la comparabilidad histórica.
...
\evidence{La implementación ejecutable de fórmulas, metas y semáforos se encuentra en \texttt{apps/analytics/metrics/calculator.py}; la evidencia consolidada se exporta a \texttt{reportes/evidencias/resultados/metricas\_resumen.json}.}


\subsection{Fase 7: obtención y validación de datos}

\scenario{7}{Fuentes, generación y calidad del dato · desarrollo aplicado}

\subsubsection{Objetivo y arquitectura de fuentes}

El objetivo es obtener datos suficientes y confiables para calcular las medidas sin modificar el conjunto original. Se establecieron tres niveles de procedencia: \textbf{original}, para los 3.000 incidentes suministrados; \textbf{simulado}, para eventos y encuestas generados con reglas declaradas; y \textbf{derivado}, para clasificaciones, agregados y KPI.
...
\midrule
Eventos cargados & 6.000 & Coincide con el manifiesto. \\
Encuestas cargadas & 2.000 & Coincide con el manifiesto. \\
...
Los controles permitieron calcular las métricas sin nulos en campos críticos, duplicados ni valores fuera de dominio. El manifiesto registra semilla, periodo y cantidades, por lo que otra persona puede regenerar la misma línea base. Esta reproducibilidad es una propiedad del estudio, no una prueba de que los valores representen una institución real.
...
\evidence{El manifiesto reproducible está en \texttt{apps/data/simulated/manifest.json}; los eventos y encuestas se generan en la misma carpeta y los 3.000 incidentes originales permanecen preservados en \texttt{apps/data/original}.}


\subsection{Fase 8: automatización de la medición}

\scenario{8}{Pipeline Python, pruebas y ejecución continua · desarrollo aplicado}
...
\midrule
\path{classification/classifier.py} & Normaliza y asigna característica principal y justificación a los incidentes. \\
\path{simulation/generator.py} & Genera eventos y encuestas reproducibles con semilla 25022. \\
\path{metrics/calculator.py} & Calcula porcentajes, P90, medias, desviación entre sedes, metas y semáforos. \\
\path{exporters/pipeline.py} & Orquesta entradas y salidas, y crea CSV y resúmenes JSON. \\
\path{tests/test_analytics.py} & Comprueba precedencia de riesgo, eficiencia, reproducibilidad y percentiles. \\
\bottomrule
\end{tabularx}
\end{table}
...
Desde la raíz del repositorio, la medición se ejecuta con:
...
La primera orden clasifica 3.000 incidentes, genera 6.000 eventos y 2.000 encuestas, calcula diez KPI y exporta resultados. La segunda ejecuta cinco pruebas unitarias. El 13 de julio de 2026 se repitió la ejecución sobre la revisión \texttt{3baf75d}: el pipeline produjo las diez métricas esperadas y las cinco pruebas finalizaron correctamente.
...
Para la integración se levantaron PostgreSQL, SQL Server, RabbitMQ, API de medición, citas, HCE, facturación y Gateway; todos alcanzaron estado \texttt{healthy}. El puerto 8080 estaba ocupado por un proyecto externo, por lo que \path{compose.test.override.yaml} publicó el Gateway en 18080 y la prueba recibió la dirección mediante \texttt{MEDISALUD\_BASE\_URL}. Esta adaptación conserva el valor 8080 como opción predeterminada y evita interferir con otros contenedores.

La prueba de carga también se regeneró en esta sesión. JMeter emitió 500 solicitudes sincronizadas a través del Gateway: 500 fueron correctas, no hubo errores, el promedio fue 1.875,7 ms, el P90 fue 2.487 ms y el P95 fue 2.510 ms. Los resultados proceden del archivo JTL creado a las 21:57 hora de Ecuador y del resumen JSON derivado, no de una estimación manual.

El primer intento de Flutter reveló una incompatibilidad reproducible: Flutter 3.41.7 incluía Dart 3.11.5, inferior al requisito \texttt{sdk: \string^3.12.2}. Se preservó ese SDK y se instaló Flutter 3.44.6/Dart 3.12.2 en paralelo. Con el entorno compatible, \texttt{pub get}, \texttt{flutter analyze}, \texttt{flutter test} y \texttt{flutter build web} finalizaron correctamente. Registrar el intento y su resolución evita ocultar una dependencia de infraestructura.

\subsubsection{Resultados de automatización}

La ejecución produjo diez métricas: tres en verde, cuatro en amarillo y tres en rojo. Los valores detallados se presentan en la Tabla \ref{tab:results}. Desde la perspectiva del pipeline, el resultado principal es la consistencia entre archivos procesados, resumen JSON y consumidores del tablero.
...
Salida & Uso \\
\midrule
\path{apps/data/processed/incidentes_clasificados.csv} & Trazabilidad individual de la clasificación. \\
\path{apps/data/processed/metricas.csv} & Resultado tabular de las diez medidas. \\
\path{reportes/evidencias/resultados/*.json} & Resumen versionable para informe y auditoría. \\
\path{reportes/evidencias/pruebas/*} & Acta y resumen estructurado de la ejecución real del 13 de julio de 2026. \\
\path{dashboards/dashboard-data.js} & Fuente regenerable del tablero académico. \\
...
El flujo \path{.github/workflows/medicion-calidad.yml} ejecuta el pipeline y sus pruebas de forma manual, en cada cambio relevante y mediante programación semanal. Los productos procesados se publican como artefactos de la ejecución. Esta automatización proporciona historial y repetibilidad; no reemplaza la revisión profesional de fuentes, umbrales y resultados.
...
\evidence{El pipeline se ejecutó con semilla 25022 y produjo diez métricas. Las seis suites sumaron 19 casos aprobados de 19 ejecutados; también aprobaron los análisis y las construcciones React y Flutter. El acta completa está en \texttt{reportes/evidencias/pruebas/ejecucion-real-2026-07-13.md}; JMeter conserva el JTL y su resumen en \texttt{reportes/evidencias/calidad/jmeter}.}
